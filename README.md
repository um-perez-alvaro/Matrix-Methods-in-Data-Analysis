# Matrix Methods in Data Analysis


This is the GitHub repository of the book *Matrix Methods in Data Analysis*

## Contents:

- [LINEAR ALGEBRA AND MACHINE LEARNING](https://github.com/um-perez-alvaro/Matrix-Algebra-for-Data-Analysis/blob/main/chapters/Chapter_1/README.md)
- [MATRIX MULTIPLICATION AND PARTITIONED MATRICES](https://github.com/um-perez-alvaro/Matrix-Algebra-for-Data-Analysis/blob/main/chapters/Chapter_2/README.md)
- [NORMS, AND DISTANCES AND SIMILARITIES]()
- [THE FOUR FUNDAMENTAL SUBSPACES OF A MATRIX AND GRAM-MATRICES]()
- [THE LU FACTORIZATION OF A MATRIX]()
- [ORTHOGONALITY AND THE QR FACTORIZATION]()
- [ORTHOGONAL PROJECTIONS AND THE LEAST SQUARES PROBLEM]()
- [EIGENVALUES, EIGENVECTORS, AND ALGORITHMS]()
- [SYMMETRIC AND POSITIVE DEFINITE MATRICES]()
- [THE SINGULAR VALUE DECOMPOSITION]()
- [NONNEGATIVE MATRICES AND PERRON THEORY]()

## Datasets
- [Datasets](https://github.com/um-perez-alvaro/Matrix-Tools-for-Data-Science/blob/main/datasets/README.md)


